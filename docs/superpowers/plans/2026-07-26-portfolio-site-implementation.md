...
- [ ] **Step 5: Write `components/icons.tsx` and `components/SocialLinks.tsx`**

> Note: the installed `lucide-react@1.27.0` no longer ships brand/logo icons (`Github`, `Linkedin` were dropped upstream) — only generic icons remain (`Mail`, `Download`, `Menu`, `X`, `Moon`, `Sun`, `ExternalLink`, etc. are all still present). Define the two brand icons once in a shared file so this task and Task 10 (which also needs a GitHub icon) don't each duplicate the SVG.
